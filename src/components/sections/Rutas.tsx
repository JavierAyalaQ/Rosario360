...
export interface FramePlayerProps {
  ...
  routes: { label: string; src: string[] }[]; // Opciones de rutas con labels y arrays de frames
...
const FramePlayer: React.FC<FramePlayerProps> = ({ frames, routes, frameRate = 30, frameDelay = 500 }) => {
  ...
  // Cambiar a una nueva ruta de frames
  const handleRouteChange = (newFrames: string[]) => {
              ...
              {routes.map((option, index) => (
                <li key={index}>
                  <button
                    ...
                    onClick={() => handleRouteChange(option.src)}
                  >
                    {option.label}
                  </button>
                </li>
